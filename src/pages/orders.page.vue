<template>
    ...
    <section class="page">
        ...
        <div class="relative mb-16">
            ...
            <orders-table :view="view" :response="response" :list="list" :clickable="true" :loading="loading_table"
...
import ordersTable from './tables/orders.table.vue';
...
function onItemClick(event: any) {
    console.log('-----clicked----', list.value[event.index].id);
    router.push({ name: 'order', params: { id: list.value[event.index].id } });
}
...
async function load() {
    ...
    try {
        var res = await new RequestHandler().get('bookings/findAllBookingsWeb',
            {
                search: search.value,
                sort: sort.value,
