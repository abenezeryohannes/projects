<template>
  <section class="page">
    ...
    <add-notification @on-change="load()" ref="addNotificationModel" />
    ...
    <div class="relative p-3 shadow-md pt-3 px-3">
      ...
      >
        <action-fab
          :loading="loading"
          action="add"
          @on-action="addNotificationModel.open()"
        />
      </div>
    </div>
  </section>
...
import notificationTable from "./tables/notification.table.vue";
...
import addNotification from "../pages/modals/add.notification.modal.vue";
import { Notification } from "../domain/user/entity/notification.entity";
let scroll = ref<number>(0);
let loading = ref<boolean>(false);
let loading_table = ref<boolean>(true);
let page = ref<number>(1);
let addNotificationModel = ref<any>();

let response = ref<WrapperDto<Notification>>(
  new WrapperDto({
    count: 0,
    page: 0,
    total_pages: 1,
    datas: [],
  } as WrapperDto<Notification>)
);

let list = ref<Notification[]>([]);
...
let view = ref<string | null>(localStorage.getItem("view_type_notification"));
// const router = useRouter();

watch(
  () => view.value,
  (val, _) => {
    if (val != null) localStorage.setItem("view_type_notification", val!);
  }
);
...
function onItemClick(_: any) {
  ...
  //   router.push({ name: "notification", params: { id: list.value[event.index].id } });
}
...
async function load() {
  ...
  try {
    const res = (
      await new RequestHandler().get("notifications/all", {
        search: search.value,
        sort: sort.value,
    ...
    ).data as WrapperDto<Notification>;
    console.log("res", res);
    response.value = res;
    if ((res.datas?.length ?? 0) > 0)
      list.value = (res as WrapperDto<Notification>).datas ?? [];
  } catch (e) {
