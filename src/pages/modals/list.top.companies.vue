...
import CompaniesTable from "../tables/top.companies.table.vue";
...
let sort_by = ref<string | null>("clicked");
let limit = ref<number | null>(8);
let sort = ref<string | null>(null);
let view = ref<string | null>(localStorage.getItem("view_type_top_business"));
...
watch(
  () => view.value,
  (val, _) => {
    if (val != null) localStorage.setItem("view_type_top_business", val!);
  }
);
