...
import addBusinessForms from "../forms/add.business.forms.vue";
...
const temp_company = ref<Company | undefined | null>({ isActive: true });
...
async function save() {
  try {
    loading.value = true;
    console.log("request body: ", temp_company.value);
    var response = await new RequestHandler().postForm(
      "companies/add",
      temp_company.value,
      [{ name: "banner", file: icon.value }]
    );
...
//     temp_company.value = response.data;
