...
import CompaniesTable from "../tables/new.companies.table.vue";
...
let sort_by = ref<string | null>("id");
let limit = ref<number | null>(8);
let sort = ref<string | null>("DESC");
let view = ref<string | null>(localStorage.getItem("view_type_new_business"));
...
watch(
  () => view.value,
  (val, _) => {
    if (val != null) localStorage.setItem("view_type_new_business", val!);
  }
);
