...
import addFeatureForms from '../forms/add.feature.forms.vue';
...
const temp_feature = ref<any>({ enabled: true });
...
async function save() {
    try {
        loading.value = true;
        var response = await (new RequestHandler()).postForm('features/store', temp_feature.value, [
...
async function load() {
    try {
        loading.value = true;
        var response = await new RequestHandler().get('features/findAllForAdmin' + route.params.id, {});
