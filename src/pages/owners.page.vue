<template>
    ...
    <section class="page">
        ...
        <div class="relative mb-16">
            ...
            <owners-table :view="view" :response="response" :list="list" :clickable="true" :loading="loading_table"
...
import ownersTable from './tables/owners.table.vue';
...
watch(() => view.value, (val) => { if (val != null) localStorage.setItem('view_type', val!); });
...
function onItemClick(event: any) {
    ...
    router.push({ name: 'owner', params: { id: list.value[event.index].id } });
}
...
async function load() {
    ...
    try {
        var res = await new RequestHandler().get('users/findAllOwners',
            {
                search: search.value,
                sort: sort.value,
