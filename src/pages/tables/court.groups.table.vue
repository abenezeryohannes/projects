        ...
        <div class="w-full text-xs">
            ...
            <div v-else>
                <grid @on-select="onSelect" @on-action="onAction" @on-page-change="emit('on-page-change', $event)"
                    @bulk-action="onBulkAction" @on-limit-change="emit('on-limit-change', $event)"
                    @on-item-click="emit('on-item-click', $event)" item="court.group" :clickable="clickable"
...
const bulkActions = ref<any>([{
    name: 'Delete All', url: "padel-group/deleteAll", title: 'Are you sure?',
    description: 'This will delete all the selected courts.'
}]);
...
const headers = ref<any[]>([
    ...
    { label: 'color', name: 'color', width: '3', sortable: true, type: "color", editable: true, cls: 'justify-start' },
    { label: 'sequence', name: 'sequence', width: '3', sortable: true, type: "text", editable: true, cls: 'justify-start' },
    {
...
async function onAction(x: { index: number, body: any, action: string, files: any[] }) {
    ...
    switch (x.action.toLowerCase()) {
        case 'delete':
            try {
                await new RequestHandler().post('padel-group/delete', { 'id': props.list![x.index].id });
                emit('update-list');
            } catch (e) { errorHandlerUtil.handle(e, useToast(), useRouter()); }
            break;
        case 'edit':
            try {
                console.log('request', x.body);
                await new RequestHandler().post('padel-group/edit',
                    x.body);
...
function mapTableData(listPar: any[]) {
    ...
    listPar.forEach((element) => {
        tableData.value[tableData.value.length] = {
            name: element.name,
            color: element.color,
            sequence: String(element.sequence),
            icon: element.icon,
            id: element.id,
            identifier: String(element.id),
