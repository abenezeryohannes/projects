        ...
        <div class="w-full text-xs">
            ...
            <div v-else>
                <grid @on-select="onSelect" @on-action="onAction" @on-page-change="emit('on-page-change', $event)"
                    @bulk-action="onBulkAction" @on-limit-change="emit('on-limit-change', $event)"
                    @on-item-click="emit('on-item-click', $event)" item="ad" :clickable="clickable"
...
const props = defineProps<{ list: any[], response?: any, view?: string | null, loading?: boolean, 
    clickable?: boolean }>();

//const { t } = useI18n();

const bulkActions = ref<any>([ {   
    name: 'Delete All', url: "ads/deleteAll", title: 'Are you sure?',
    description: 'This will delete all the selected ads.' }
]);
...
const headers = ref<any[]>([
    ...
    {   
        label: 'banner', name: 'banner', width: '3', sortable: false, type: "image", 
        cls: 'justify-start', editable: false 
    },
    //{ label: 'link', name: 'link', width: '3', sortable: false, type: "image", cls: 'justify-start', editable: false },
    {
        label: 'enabled', name: 'enabled', width: '3', sortable: true, type: "toggle", option: { positive: 'yes', negative: 'no' },
...
async function onAction(x: { index: number, body: any, action: string }) {
    ...
    switch (x.action.toLowerCase()) {
        case 'delete':
            try {
                await new RequestHandler().post('ads/delete', { 'id': props.list![x.index].id });
                emit('update-list');
            } catch (e) {
                errorHandlerUtil.handle(e, useToast(), useRouter());
            }
        ...
        case 'edit':
            try {
                ...
                var response = await new RequestHandler().post('ads/edit', x.body);
