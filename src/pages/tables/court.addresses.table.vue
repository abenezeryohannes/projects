...
const bulkActions = ref<any>([
    //     {
    //     name: 'Delete All', url: "addresses/deleteAll", title: 'Are you sure?',
    //     description: 'This will delete all the selected addresses.' 
    // }
]);
...
const headers = ref<any[]>([
    ...
    {
        label: 'name', name: 'name', width: '3', sortable: true, type: "text", editable: true,
        cls: 'justify-start'
    },
    {
        label: 'enabled', name: 'enabled', width: '2', sortable: true, type: "toggle",
        option: { positive: 'true', negative: 'false' },
        cls: 'justify-start', editable: true
    },
    // {label:'type', name:'indoor', width:'2', sortable:true, type:"toggle",option:{positive: 'indoor', negative: 'outdoor'},
    //  editable: true,cls: 'justify-start'}, 
    //  {label:'durations', name:'duration', width:'2', sortable:false, type:"text",  cls: 'justify-start', editable: false}, 
    // {label:'price', name:'price', width:'2', sortable:true, type:"cash", editable: true, cls:'justify-start'}, 
    // {label:'status', name:'status', width:'2', sortable:true, cls:'justify-center text-center', type:"status", }, 
    { label: 'action', name: '#actions#', width: '2', sortable: false, actions: ['edit', 'delete'] },
]);
...
async function onBulkAction(x: { selected: any, action: any }) {
    ...
    try {
        let response = await new RequestHandler().post(x.action.url, { 'ids': JSON.stringify(ids) });
...
async function onAction(x: { index: number, body: any, action: string }) {
    ...
    switch (x.action.toLowerCase()) {
        case 'delete':
            try {
                await new RequestHandler().post('addresses/delete', { 'id': props.list![x.index].id });
                emit('update-list');
            } catch (e) {
                errorHandlerUtil.handle(e, useToast(), useRouter());
            }
        ...
        case 'edit':
            try {
                ...
                var response = await new RequestHandler().post('addresses/edit',
                    x.body);
