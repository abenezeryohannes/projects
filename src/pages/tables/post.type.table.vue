...
const bulkActions = ref<any>([
    {
        name: 'Delete All', url: "postGroups/deleteAll", title: 'Are you sure?',
        description: 'This will delete all the selected postGroups.'
    }
]);

const headers = ref<any[]>([
    { name: '#bulk#', width: '1', sortable: false },
    { label: 'icon', name: 'icon', width: '2', sortable: false, type: "text", cls: 'justify-start', editable: false },
    { label: 'name', name: 'name', width: '4', sortable: true, type: "text", editable: true, cls: 'justify-start' },
    { label: 'color', name: 'name', width: '3', sortable: true, type: "text", editable: true, cls: 'justify-start' },
    { label: 'sequence', name: 'sequence', width: '3', sortable: true, type: "text", editable: true, cls: 'justify-start' },
    {
        label: 'enabled', name: 'enabled', width: '2', sortable: true, type: "toggle", option: { positive: 'indoor', negative: 'outdoor' },
...
async function onAction(x: { index: number, body: any, action: string }) {
    ...
    switch (x.action.toLowerCase()) {
        case 'delete':
            try {
                await new RequestHandler().post('postGroups/delete', { 'id': props.list![x.index].id });
                emit('update-list');
            } catch (e) {
                errorHandlerUtil.handle(e, useToast(), useRouter());
            }
        ...
        case 'edit':
            try {
                ...
                var response = await new RequestHandler().post('postGroups/edit', x.body)
