...
const props = defineProps<{
  list: ChatDocument[];
  response?: WrapperDto<ChatDocument>;
  view?: string | null;
  loading?: boolean;
  clickable?: boolean;
}>();
...
const tableData = ref<ChatDocument[]>([]);
const gridData = ref<ChatDocument[]>([]);
...
async function onAction(x: {
  ...
  switch (x.action.toLowerCase()) {
    case "delete":
      try {
        await new RequestHandler().post("chatdocuments/delete", {
    ...
    case "edit":
      try {
        var response = await new RequestHandler().post(
          "chatdocuments/" + props.list![x.index].id + "/edit",
          x.body
        );
...
function mapTableData(listPar: ChatDocument[]) {
  tableData.value = [];
  listPar.forEach((element) => {
    let index = tableData.value.length;
    element.senderFullName = element.sender?.fullName;
    element.senderAvatar = element.sender?.avatar;
    tableData.value[index] = element;
    // tableData.value[index].banner = constants.MEDIA_LINK + element.banner;
  });
}

function mapGridData(listPar: ChatDocument[]) {
  gridData.value = [];
  listPar.forEach((element) => {
...
function map(newValue: ChatDocument[]) {
