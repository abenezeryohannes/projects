...
const headers = ref<any[]>([
                ...
                {label:'Full Name', name:'fullName', width:'2', sortable:true, type:"text", editable: true, 
                        cls: 'justify-start'}, 
                {label:'Email Address', name:'emailAddress', width:'3', sortable:false, type:"text", 
                        cls: 'justify-start', editable: false}, 
                {label:'Phone Number', name:'phoneNumber', width:'3', sortable:true, type:"text", 
                        editable: true, cls:'justify-start'}, 
                {label:'enabled', name:'enabled', width:'2', sortable:true, cls:'justify-center text-center', 
                        type:"toggle", option:{positive: 'Yes', negative: 'No'}, }, 
...
async function onAction(x: { index: number, body: any, action: string }) {
    ...
    switch (x.action.toLowerCase()) {
        case 'delete': 
            try {
                await new RequestHandler().post('users/delete', { 'id': props.list![x.index].id });
                emit('update-list');
            } catch (e) {
                errorHandlerUtil.handle(e, useToast(), useRouter());
            }
        ...
        case 'edit': 
            try {
                ...
                var response = await new RequestHandler().post('users/edit' ,
                    x.body);
