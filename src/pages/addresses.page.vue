<template>
  <section class="page">
    ...
    <add-address @on-change="load()" ref="addAddressModal" />
    ...
    <div class="relative mb-16">
      ...
      >
        <action-fab
          :loading="loading"
          action="add"
          @on-action="addAddressModal.open()"
        />
      </div>
    </div>
  </section>
...
import courtAddressesTable from "./tables/court.addresses.table.vue";
import RouteIndicator from "../components/common/route.indicator.vue";
import { useRoute, useRouter } from "vue-router";
import { onBeforeUnmount, onMounted, ref, watch } from "vue";
import { useToast } from "vue-toastification";
import addAddress from "./modals/add.address.vue";
...
async function load() {
  ...
  try {
    var res = await new RequestHandler().get("addresses/findAllForAdmin", {
      search: search.value,
      sort: sort.value,
