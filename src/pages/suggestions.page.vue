<template>
  <section class="page">
    ...
    <add-suggestion @on-change="load()" ref="addSuggestionModel" />
    ...
    <div class="relative p-3 pt-3 px-3">
      ...
      >
        <action-fab
          :loading="loading"
          action="add"
          @on-action="addSuggestionModel.open()"
        />
      </div>
    </div>
  </section>
...
import suggestionTable from "./tables/suggestion.table.vue";
...
import addSuggestion from "../pages/modals/add.business.vue";
import { Suggestion } from "../domain/chat/entity/suggestion.entity";

let scroll = ref<number>(0);
let loading = ref<boolean>(false);
let loading_table = ref<boolean>(true);
let page = ref<number>(1);
let addSuggestionModel = ref<any>();

let response = ref<WrapperDto<Suggestion>>(
  new WrapperDto({
    count: 0,
    page: 0,
    total_pages: 1,
    datas: [],
  } as WrapperDto<Suggestion>)
);

let list = ref<Suggestion[]>([]);
...
let view = ref<string | null>(localStorage.getItem("view_type_suggestion"));
// const router = useRouter();

watch(
  () => view.value,
  (val, _) => {
    if (val != null) localStorage.setItem("view_type_suggestion", val!);
  }
);
...
function onItemClick(_: any) {
  ...
  //   router.push({ name: "suggestion", params: { id: list.value[event.index].id } });
}
...
async function load() {
  ...
  try {
    const res = (
      await new RequestHandler().get("suggestions", {
        search: search.value,
        sort: sort.value,
    ...
    ).data as WrapperDto<Suggestion>;
    console.log("res", res);
    response.value = res;
    if ((res.datas?.length ?? 0) > 0)
      list.value = (res as WrapperDto<Suggestion>).datas ?? [];
  } catch (e) {
