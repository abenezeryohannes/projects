  ...
  <section>
    <div class="w-full">
      <div class=" " v-for="(body, index) in tableData" :key="index">
        <table-row @on-action="emit('on-action', 
        {index:index, body:$event.body, item_body: $event.item_body, action:$event.action, orginal:$event.orginal})"
          @on-select="emit('on-select', {index: index, check: $event.check} )"
          @on-item-click="emit('on-item-click', $event)" :item="item" :checkAll="checkAll" :clickable="clickable"
          :show_item_on="show_item_on" :unCheckAll="unCheckAll" :data="body" :headers="headers"
          :error="parseError(index)" :index="index" />
      </div>
    </div>
  </section>
</template>
...
function value(index: number, body: any[], header: any) {
  return (editing.value != index) ? body[header.name] : temp_tableData.value[header.name]
}

function update(key: string, value: any) { console.log(key, value); temp_tableData.value[key] = value; console.log(temp_tableData.value) }
