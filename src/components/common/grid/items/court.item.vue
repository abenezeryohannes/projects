<template>
    <section>
        <div class="relative w-full rounded hover:bg-accentlight-light dark:bg-accentlight-dark bg-background-light 
          ...
          <div class="relative col-span-10 font-semibold pr-3 pb-1 text-left capitalize" :class="[{'cursor-pointer': clickable}]" @click="onClick">
  
                <p class="  mb-1 mt-1 truncate text-sm mr-10"> {{data.group}} </p>
   
                  <div class="flex mb-2 font-bold text-sm truncate">
                      <p class=""> {{data.name}} </p>
                      
                      <!-- <p class="text-gray-400"> &nbsp; By  &nbsp; </p> -->
                  ...
                  <!-- <div class="flex mb-2">
                      <p v-if="data.indoor" class="text-blue-500"> Indoor </p>
                      <p v-else class="text-purple-200"> Outdoor </p>
                      <p v-if="data.ladies" class="text-green-500"> Ladies </p>
                  </div> -->
                  
                  <div class="absolute bottom-1  flex">
                      <p class="py-1 text-base"> {{fM(data.price)}} </p>
                  </div>
    
                  <div class=" absolute bottom-2 right-2 flex ">
                    <div class="font-light capitalize" :class="[
                        // { 'bg-warning-light dark:bg-warning-dark rounded-2xl shadow-xl px-2 py-1 ': data.status.includes('issue') },
                        { 'text-warning-light justify-centerpx-2 py-1 ': data.status.includes('waiting') },
                        { 'text-error-light justify-centerpx-2 py-1 ': data.status.includes('in-active') },
                        ...
                        { 'text-success-light justify-centerpx-3 py-1 ': data.status == ('active') }
                    ]">
