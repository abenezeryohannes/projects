<template>
  <section>
      <div class="w-full rounded hover:bg-accentlight-light dark:bg-accentlight-dark bg-background-light dark:hover:bg-background-darkish shadow-lg grid grid-cols-12 gap-x-3">
        ...
        <div class="col-span-8 font-semibold relative pr-3 pb-1 text-left capitalize" 
                :class="[{'cursor-pointer': clickable}]" @click="onClick">

              <p class="  mb-1 mt-2 truncate text-sm"> {{data.name}} </p>
                ...
                <div class="flex mb-2">
                    <p class="mr-2"> {{$t('cost')}}: </p>

                    <p class="text-red-600"> {{$date.fM(data.cost)}} </p>
                </div>
 
                <div v-show="data.can_sell" class="flex mb-2">
                    <p class="mr-2"> {{$t('sale_price')}}: </p>

                    <p class="text-green-600"> {{$date.fM(data.sale_price)}} </p>
                </div> 
