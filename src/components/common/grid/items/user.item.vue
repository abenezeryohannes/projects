<template>
  <section>
    <div
      class="w-full rounded hover:bg-accentlight-light dark:bg-accentlight-dark bg-background-light dark:hover:bg-background-darkish shadow-lg grid grid-cols-12 gap-x-3"
    >
      ...
      <div
        class="col-span-8 relative pr-3 pb-1"
        :class="[{ 'cursor-pointer': clickable }]"
        @click="onClick"
      >
        <div v-for="(header, header_index) in headers" :key="header_index">
          <div
            v-if="header.label != '#bulk#' && header.name != '#actions#'"
            class="flex text-gray-700 dark:text-white"
          >
            <!-- data area -->
            <div class="text-left my-auto text-xs font-semibold truncate py-1">
              <p v-if="header.type == null" class="truncate">
                {{ $t(data[header.name]) }}
              </p>
              ...
              <dropdown-form
                @on-change="temp_data[header.name] = $event.name"
              ...
              <toggle-form
                ...
                :editing="editing"
                :positive="header.option.positive"
                :negative="header.option.negative"
                :width="4"
                :height="4"
                :toggle="value(data, header, error)"
                @on-change="temp_data[header.name] = $event"
              />
            </div>
          </div>
        ...
        <!-- actions area -->
        <div
          v-if="hasCol('#actions#') && false"
          class="absolute bottom-2 right-0 flex flex-row justify-end"
        >
          <div v-for="action in getActions()" :key="action">
            <div
              v-show="!editing && error == null"
              @click="onActionClick(data, action)"
            >
              <v-icon
                cls="cursor-pointer mr-2 my-auto dark:text-accent-light"
                :padding="0"
                fill="gray-600"
                :icon="action"
                :size="5"
          ...
          <div
            v-show="editing || error != null"
            @click="onActionClick(data, 'cancel')"
          >
            <v-icon
