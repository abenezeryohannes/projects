<template>
  <section>
      <div class="relative w-full rounded hover:bg-accentlight-light dark:bg-accentlight-dark 
                bg-background-light dark:hover:bg-background-darkish shadow-lg grid grid-cols-16 gap-x-3">
        <div class="static col-span-6 bg-background-lightish dark:bg-background-darkish">
            <web-image class="w-full h-32 " rounded="rounded" :width="2999"  :link="data.avatar" :padding=0 />
        </div>
        ...
        <div class="col-span-10 font-semibold relative pr-3 pb-1 text-left capitalize" :class="[{'cursor-pointer': clickable}]" @click="onClick">
              ...
              <p class="  mb-1 mt-2 truncate text-sm mr-10"> {{data.fullName}} </p>
                ...
                <div class=" flex mb-2 ">
                    <div class="font-light capitalize" >
                        ...
                        <p v-else class="text-error-dark justify-centerpx-3 py-1"> {{ 'In-Active' }} </p>
                    </div>
                </div>
