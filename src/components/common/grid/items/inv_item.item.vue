<template>
  <section>
      <div class="relative w-full flex text-xs hover:bg-accentlight-light dark:bg-accentlight-dark rounded-lg bg-background-light dark:hover:bg-background-darkish shadow-lg  ">
        ...
        <div class="bg-transparent pl-1 cursor-pointer" v-show="data.image!=null">
          <web-image class="w-24 h-32" scale="object-contain" rounded="rounded" :link="data.image" :padding=0 />
        </div>

        <div class="w-24 h-35 bg-gray-100 cursor-pointer dark:bg-background-dark" v-show="data.image == null"/>

        <div class="pb-2 pt-1 px-3 w-full font-semibold relative pr-3 pb-1 text-left capitalize" :class="[{'cursor-pointer': clickable}]" @click="onClick">

              <p class="  mb-1 mt-2 truncate text-xs"> {{data.name}} </p>
 
                <div class="flex mb-2 text-ty truncate">
                    <!-- <p class="mr-2"> {{$t('manager')}}: </p> -->
                    <p class=""> {{data.sku}} {{ (data.variations.length>0)?'-':''}} {{data.variations}} </p>
                </div>

                <div class="flex text-ty mb-2  truncate">
                    <!-- <p class="mr-2"> {{$t('manager')}}: </p> -->
                  <tag-input-form cls="text-xt text-xt border rounded-full mx-1 px-2 py-1"
                                  :tags="data.location" :deletable="false" :editing="false" :value="data.location" />
<!--                    <p class=""> {{data.location}} </p>-->
                </div>

<!--                 <div class="flex mb-2">-->
<!--                    <p class=""> {{$date.fM(data.cost)}} </p>-->
<!--                </div>-->
...
<!--                                    ]">-->
                            <p class=""> {{$date.number(data.quantity_on_hand) + ' ' + $t(data.unit)}} </p>
<!--                    </div>-->
                </div>
...
import TagInputForm from "@/components/forms/tag.input.form";
export default {
  components: {TagInputForm, WebImage,   CheckboxForm },
