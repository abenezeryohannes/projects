<template>
  <section>
      <div class="relative w-full  text-xs hover:bg-accentlight-light dark:bg-accentlight-dark rounded-lg bg-background-light dark:hover:bg-background-darkish shadow-lg  ">
        ...
        <div class="pb-2 pt-1 px-3 font-semibold relative pr-3  text-left capitalize" :class="[{'cursor-pointer': clickable}]" @click="onClick">

              <p class="  mb-1 mt-2 truncate text-sm"> {{data.reference}} </p> 

                <div class="flex mb-1 mt-2 truncate"> <!-- <p class="mr-2"> {{$t('manager')}}: </p> -->
                    <p class=""> {{data.tenant}} </p>
                </div> 

                <div class="flex mb-2 text-xs truncate"> <!-- <p class="mr-2"> {{$t('manager')}}: </p> -->
