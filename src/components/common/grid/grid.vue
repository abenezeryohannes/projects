<template>
    <section>
        ...
        <div class="w-full">
            <div class="">
                <grid-header :unCheckAll="unCheckAll" :checkAll="checkAll" :bulkActions="bulkActions"
                    @bulk-select-action="onBulkSelectAction($event)" :selected="selected" @bulk-action="onBulkAction"
                    :headers="headers" :selecting="selecting" @on-select="onSelectAll" />
            </div>

            <div>
                <!-- Loader when no data and loading is true -->
                <div v-if="(loading && (list == null || list.length == 0))" id="loading-screen" class=" w-full h-full">
                    ...
                    <div :class="[{ 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 gap-y-10': cols == null }, cols]"
                        class="w-full grid">
                        <div class="rounded animate-pulse h-28 dark:bg-accentlight-dark bg-accentlight-light mt-2 "
                            v-for="i in 12" :key="i" />
                    </div>
                </div>
                <!-- grid body -->
                <grid-body :item="item" :cols="cols" @on-item-click="emit('on-item-click', $event)"
                    :clickable="clickable" :unCheckAll="unCheckAll" :errors="errors" @on-action="onAction"
                    :headers="headers" :tableData="list" :checkAll="checkAll" @on-select="onSelect" />
            </div>

            <div class="pt-8">
                <pagination :response="response" :limit_controller="true"
                    @on-limit-change="emit('on-limit-change', $event)" @on-change="emit('on-page-change', $event)"
...
function onBulkSelectAction(x: { name: string, url: string, title: string, description: string, onSelect: any, }) {
    // console.log({name, url, title, description, onSelect})    
    if (x.onSelect) onBulkAction(x);
}

function onBulkAction(x: { name: string, url: string, title: string, description: string, onSelect: any, }) {
    if (x.name == 'cancel') {
        clearSelection();
    }
    else if (x.name == null || (x.title == null && x.description == null)) {
        emit('bulk-action', {
            selected: JSON.parse(JSON.stringify(props.selected?.filter((x) => { return x != null }))),
            action: x
        });
    } else {
...
function onAction(x: { index: number, body: any, item_body: any, action: string, orginal: any }) {
    if (x.action == 'delete') {
        modal.value.open(
            {
                id: 'action',
...
function onSelectAll(check: boolean) {
    checkAll.value = check; unCheckAll.value = false;
...
function clearSelection() {
    checkAll.value = false; unCheckAll.value = true;
    selected.value.splice(0, selected.value.length)
}
...
function onmodalResponse(x: { id: string, action: string, data: any }) {
    if (x.id == 'bulk_action') {
        if (x.action == 'yes') {
            emit('bulk-action', { selected: JSON.parse(JSON.stringify(selected.value.filter((x) => { return x != null }))), action: x.data });
            clearSelection();
        }
    } else if (x.id == 'action') {
        if (x.action == 'yes') {
            if (x.data.action == 'delete')
                emit('on-action', x.data);
        }
    }
}
...
watch(() => selecting.value, (newVal, oldVal) => {
