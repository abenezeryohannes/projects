  ...
  <section class=" ">
    <div v-if="!editing" class="capitalize">
      <p :class="[{ 'text-gray-400': min == null || max == null }]">
        {{ min == null ? $t("select_start_date") : $date.dd(min, "s") }} -
        {{ max == null ? $t("select_end_date") : $date.dd(max, "s") }}
...
export default {
  ...
  computed: {
    calendar() {
      return this.$store.getters.user.calendar;
    },
    error() {
      return errorHandlerUtil.parse(this.errors, this.name);
    },
  },
};
</script>
