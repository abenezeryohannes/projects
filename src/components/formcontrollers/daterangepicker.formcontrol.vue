<template>
  <section>
      <div class="flex " :class="{'': error==null}">
          <label v-if="label!=null" class=" capitalize " :class="[{'my-auto':error == null}, 'w-'+(gap==null?40:gap)]"> {{t(label.toLowerCase())}}: </label>

          <daterangepicker-form :gravity="gravity" :class="[cls]" class="w-full" :min="min" :max="max" :errors="errors" :name="name" 
