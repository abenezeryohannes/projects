<template>
  <section>
    <div :class="[{ ' ': direction === 'vertical' }, { 'flex': direction !== 'vertical' }, { 'flex-none': error == null }]">
      ...
      <number-form :min="min" :max="max" :type="type" :class="[value_cls]" class="w-full"
        @on-change="emit('on-change', $event)" :placeholder="placeholder" :value="value" :error="error" :name="name"
        :editing="editing" />
    </div>
